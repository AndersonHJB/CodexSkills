...
- `generate-personal-ip-avatars`：用户只需提供一张人物参考照，即可生成快速 8 张经典组，或完整 9 组共 72 张个人 IP 插画；自动适配人物的视觉呈现，并自主设计 8 套新主题色、动作和标志性配饰。
...
`generate-personal-ip-avatars` 会从一张清晰人物参考照中提取脸型、五官、发长、发质、轮廓、年龄感、姿态和真实配饰，再把这些特征转译为统一手绘语言的方形个人 IP。它支持三种工作方式：快速生成经典组 8 张；从零生成经典组加 8 个新主题组、共 72 张；或在已有经典 8 张的基础上保持原图不变，只新增 64 张。
...
- 黄色与钴蓝只属于经典基准组，不会被误判成用户偏好；完整模式会额外设计 8 套有明确背景色、线条色、基础色和强调色的主题系统；
- 每个主题内的 8 张图分别改变发型处理、动作、服装细节和标志性配饰，64 张扩展图也会先建立动作与道具矩阵，不以简单换色冒充新方案；
- 已有经典 8 张时先冻结基准，新增内容写入独立目录，不覆盖、不重命名、不重新压缩原图；
- 每张图独立生成，并可使用 ImageMagick 自动制作每组 8 图总览和 9 系列总览；
...
保留经典组 8 张，再自主设计 8 套不同主题色；每套生成 8 张不同发型处理、动作、服装和标志性配饰的独立图片，共 72 张。
请自动适配人物呈现，不要从风格参考图复制不属于这个人的性别特征、配饰或颜色偏好。
```

如果只需要快速版本，可明确说“仅生成经典组 8 张”。如果已经有一组满意的 8 张，可要求“保持现有 8 张不变，只扩展 8 个新主题组”，Skill 会仅生成 64 张新增图片。

可选依赖：安装 ImageMagick 后会自动制作每组 8 图总览和完整 9 系列总览；没有 ImageMagick 时仍会交付所有独立图片。
