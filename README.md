...
- `generate-personal-ip-avatars`：为没有绘画和设计基础的用户打造专属 IP 角色盲盒；只需一张人物参考照，即可获得经典 8 张和每次随机抽取的 8 套主题色共 72 张，用户指定的喜欢色会继续追加。
...
一次生成大量方案并不是浪费生成能力，而是把插画设计中的试色、动作提案和配饰探索一次性可视化，让用户像开盲盒一样快速比较并选出值得继续精修的方向。完整中文说明见 [`skills/generate-personal-ip-avatars/README.md`](skills/generate-personal-ip-avatars/README.md)。
...
- 用户说明喜欢的颜色或主题时，会在随机 8 套之后追加对应主题，每个追加主题再生成 8 张，不会替换随机盲盒；
- 每个主题内的 8 张图分别改变发型处理、动作、服装细节和标志性配饰，默认 64 张扩展图也会先建立动作与道具矩阵，不以简单换色冒充新方案；
- 已有经典 8 张时先冻结基准，新增内容写入独立目录，不覆盖、不重命名、不重新压缩原图；
- 每张图独立生成，并使用 ImageMagick 制作每组 8 图总览和包含全部交付原图的最终拼图；
- 最终保存独立原图、各组总览、完整 `PROMPTS.md` 和可审计的 `QA.md`。
...
保留经典组 8 张，再随机抽取 8 套不同主题色；每套生成 8 张不同发型处理、动作、服装和标志性配饰的独立图片，共 72 张。
请自动适配人物呈现，不要从风格参考图复制不属于这个人的性别特征、配饰或颜色偏好。
最后把全部原图拼成一张总览图，方便我直接比较和挑选。
```

如果只需要快速版本，可明确说“仅生成经典组 8 张”。如果已经有一组满意的 8 张，可要求“保持现有 8 张不变，只扩展 8 个随机主题组”，Skill 会仅生成 64 张新增图片。用户指定喜欢色时，每个主题会在现有数量上再增加 8 张。

可选依赖：安装 ImageMagick 后会自动制作每组 8 图总览和全部图片拼图；没有 ImageMagick 时仍会交付所有独立图片。
