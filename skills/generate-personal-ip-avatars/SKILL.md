...
description: "Design a gender-adaptive personal-IP cartoon collection from one user-provided portrait. Supports a quick classic set of 8 images or a complete 9-series collection of 72 images: one immutable classic set plus eight newly art-directed color themes, each with eight distinct hairstyles, actions, outfits, and signature accessories. Use for personal IP, cartoon identity, avatar series, character packs, multi-palette explorations, or expansion of an existing approved 8-image set."
---
...
- **Full mode:** the classic eight plus eight additional color-theme series of eight images each, for 72 separate images total.
- **Expansion mode:** when an approved classic eight already exists, treat it as immutable and generate only the 64 new images. Never overwrite or silently revise the approved eight.
...
5. Choose the mode from the request:
   ...
   - Use full mode for a complete pack, multiple themes, broad color exploration, or an unspecified request to batch-design a reusable IP collection.
...
7. For full or expansion mode, read `references/theme-expansion.md` completely. Design eight genuinely distinct palettes and a 64-cell concept matrix before generation.
8. Use the built-in image-generation tool. Pass the user photo plus both bundled style assets as references. Issue one independent call per concept; never request a grid as a substitute for separate assets.
9. Generate in batches of four when parallel calls are supported. Keep the user informed between batches.
10. Validate every image against the checklist below. Regenerate only failed concepts with a targeted correction.
11. Save non-destructively:
    - Quick mode: `output/personal-ip-avatars/` or the next versioned sibling.
    - Full mode: `00-classic/` followed by `01-<theme>/` through `08-<theme>/`.
    - Expansion mode: preserve the existing classic directory and write the 64 additions to a new sibling directory.
12. Create one `00-overview.png` per eight-image series with `scripts/make_contact_sheet.sh`. For a full collection, create a nine-series overview with `scripts/make_series_overview.sh`.
13. Save resolved identity, style lock, palettes, and all concept prompts as `PROMPTS.md`. Save counts and validation results as `QA.md`.

## Validation checklist

Check all of the following before delivery:

- Quick mode has exactly 8 separate square images; full mode has exactly 72; expansion mode has exactly 64 new images plus an explicit reference to the unchanged classic 8.
...
- The eight exploration palettes are distinguishable from the classic palette and from one another. They are coordinated color systems, not superficial hue filters.
...
- Across the 64-image expansion, core props and actions do not repeat unless the user requests repetition.
...
Both scripts require ImageMagick and never modify source images.
